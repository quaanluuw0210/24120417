...
void removeTail(List*& L)
{
	...
	while (t != NULL && t->p_next != NULL && t->p_next != L->p_tail)
	{
		t = t->p_next;
	}
	NODE* temp = L->p_tail;
...
void removeBefore(List*& L, int val)
{
	...
	NODE* t = L->p_head;
	if (t == L->p_head && t->p_next == NULL)
	{
		return;
	}
	if (t == L->p_head && t->p_next->key == val)
	{
		removeHead(L);
		return;
	}
	while (t != NULL && t->p_next != NULL && t->p_next->p_next != NULL && t->p_next->p_next->key != val)
	{
		t = t->p_next;
	}
	if (t->p_next != NULL && t->p_next->p_next != NULL && t->p_next->p_next->key == val)
	{
		NODE* temp = t->p_next;
		if (temp != NULL) {
			t->p_next = temp->p_next;
			delete temp;
		}
	}
	return;
...
void removeAfter(List*& L, int val)
{
	// Your code here //
	NODE* t = L->p_head;
	if (t == NULL || t->p_next == NULL)
	{
		return;
	}
	if (t->p_next == L->p_tail && t->key == val)
	{
		removeTail(L);
		return;
	}
...
bool addPos(List*& L, int data, int pos)
{
	...
	NODE* t = L->p_head;
	int index = 0;
	while (t->p_next != NULL && index != pos - 1)
	{
		t = t->p_next;
		index++;
	}
	if (index != pos - 1 || t == NULL)
	{
		return false;
	}
	if (t == L->p_tail)
...
bool addBefore(List*& L, int data, int val)
{
	...
	while (t != NULL)
	{
		if (t->p_next != NULL && t->p_next->key == val)
		{
			break;
		}
		t = t->p_next;
...
bool removeElement(List*& L, int key)
{
	...
	while (t != NULL)
	{
		if (t->p_next != NULL && t->p_next->key == key)
		{
			break;
		}
		t = t->p_next;
